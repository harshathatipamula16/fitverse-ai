...
export function expandFashionQuery(prompt) {
  if (!prompt) return 'full body outfit';
  const lower = prompt.toLowerCase().trim();
  
  // Specific direct replacements from instructions (Rule 2)
  if (lower === 'old money' || lower.includes('old money')) {
    return 'old money outfit full body model fashion';
  }
  if (lower === 'korean streetwear' || lower.includes('korean streetwear')) {
    return 'korean streetwear full body outfit model';
  }
  if (lower === 'wedding outfit' || lower.includes('wedding outfit') || lower === 'wedding glam') {
    return 'wedding dress full body fashion model';
  }

  // Handle specific user-categories inputs (Rule 6 Mapping)
  if (lower === 'dresses' || lower === 'dress') {
    return 'dress outfit full body fashion model';
  }
  if (lower === 'streetwear') {
    return 'streetwear outfit model full body clothing';
  }
  if (lower === 'formal') {
    return 'formal suit dress runway clothing full body model';
  }
  if (lower === 'wedding') {
    return 'wedding dress full body fashion model';
  }
  if (lower === 'casual') {
    return 'stylish casual outfit full body model clothing';
  }
  if (lower === 'y2k') {
    return 'y2k streetwear full body outfit model';
  }
  if (lower === 'korean') {
    return 'korean streetwear full body outfit model';
  }
  if (lower === 'luxury') {
    return 'luxury designer clothing runway dress full body model';
  }
  if (lower === 'barbiecore') {
    return 'barbiecore dress full body fashion outfit model';
  }

  // Rule 7 AI Outfit Search: User types "black korean hoodie" -> "black korean hoodie full body outfit model"
  let enhanced = lower;
  if (!enhanced.includes('outfit') && !enhanced.includes('model') && !enhanced.includes('clothing') && !enhanced.includes('dress') && !enhanced.includes('runway') && !enhanced.includes('portrait')) {
    enhanced += ' full body outfit model';
  } else {
    if (!enhanced.includes('outfit') && !enhanced.includes('dress') && !enhanced.includes('clothing')) {
      enhanced += ' outfit clothing';
    }
    if (!enhanced.includes('model') && !enhanced.includes('portrait')) {
      enhanced += ' full body model';
    }
  }
 ...
 */
export function filterFashionPhotos(photos) {
...
export async function searchFashion(searchPrompt, isFallbackAttempt = false) {
  ...
  try {
    ...
    const filtered = filterFashionPhotos(photos);
