...
import { Menu, X, Sparkles, MessageSquare, Heart, Activity } from 'lucide-react';

export default function Navbar() {
  const { activeTab, setActiveTab, user, chatOpen, setChatOpen } = useApp();
      ...
      {mobileMenuOpen && (
        <div className="lg:hidden bg-white border-t border-pink-250 pb-4 px-4 py-4 space-y-3 shadow-lg" id="mobile_navbar_drawer">
          ...
          <div className="pt-4 border-t border-pink-100 flex flex-col gap-3">
            ...
            {user && (
              <div
                className="flex items-center gap-2 cursor-pointer p-2 bg-pink-50/20 rounded-xl border border-pink-100/30"
                onClick={() => {
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-xs text-stone-500">Couture Curator (Guest)</p>
                </div>
              </div>
            )}
          </div>
